    ...
    with open(fastq_path, 'r') as file:
        for record in SeqIO.parse(file, "fastq"):
            ...
            if read_count % image_height == image_height - 1:
                imageio.imwrite(f"{output_path}_base_{read_count // image_height + 1}.png", base_image_block, format='PNG', compress_level=9)
                imageio.imwrite(f"{output_path}_quality_{read_count // image_height + 1}.png", quality_image_block, format='PNG', compress_level=9)
        ...
        if read_count % image_height > 0:
            ...
            imageio.imwrite(f"{output_path}_base_{read_count // image_height + 1}.png", final_base_image_block, format='PNG', compress_level=9)
            imageio.imwrite(f"{output_path}_quality_{read_count // image_height + 1}.png", final_quality_image_block, format='PNG', compress_level=9)
