...
# 定义输入和输出文件夹路径
input_folder = r'D:\pythonProject\fastqtobmp\input\quality_映射'
...
rule_table.sort(
    key=lambda x: (sort_order.index(x[0]), sort_order.index(x[1]), sort_order.index(x[2]), sort_order.index(x[3])))


# 将规则保存至 Rule.txt 文件
with open('Rule.txt', 'w') as file:
    for rule in rule_table:
        file.write(f"{rule[0]}, {rule[1]}, {rule[2]}, {rule[3]}\n")
