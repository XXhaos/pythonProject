...
# 定义输入和输出文件夹路径
input_folder = r'D:\pythonProject\fastqtobmp\input\test'
        ...
        for j in range(0, G.shape[1]):
            if i >= (G.shape[0] // 4) or j >= (G.shape[1] // 4):
                ...
                if top_rule is not None:
                    ...
                    # 更新规则字典中该规则的频率
                    rules_dict[top_rule] += 1

    # 构建输出文件路径
    output_name = os.path.splitext(tiff_file)[0]
    g_file_path = os.path.join(g_output_folder, f'{output_name}_g.txt')
    g_prime_file_path = os.path.join(g_prime_output_folder, f'{output_name}_g_prime.txt')

    # 将G和G_prime矩阵转换为txt文件
    np.savetxt(g_file_path, G, fmt='%d')
    np.savetxt(g_prime_file_path, G_prime, fmt='%d')
